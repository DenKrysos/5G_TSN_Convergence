...
/*
	// General ITTI message logging (without RRC_SUBFRAME_PROCESS, which is sent every subframe (1 ms))
    // ==============================================
    // ==============================================
    uint32_t length=1;
    if ((message->ittiMsgHeader.messageId)==UDP_DATA_IND || (message->ittiMsgHeader.messageId)==UDP_DATA_REQ){
    	udp_data_ind_t *udp_data_ind_p = &message->ittiMsg.udp_data_ind;
    	length=udp_data_ind_p->buffer_length;
    }


    if ((message->ittiMsgHeader.messageId) != (RRC_SUBFRAME_PROCESS))
    	printf("[ITTI] sent message %s from %s to %s, size %u\n",
    			itti_get_message_name(message->ittiMsgHeader.messageId),
    			itti_get_task_name(message->ittiMsgHeader.originTaskId),
    			itti_get_task_name(message->ittiMsgHeader.destinationTaskId),
    			length);
    // ==============================================
    // ==============================================
*/
  ...
  int itti_send_msg_to_task(task_id_t destination_task_id, instance_t instance, MessageDef *message) {
	...
	// copy original Message to a newly allocated message
	// ======================
	MessageDef * icm_message=itti_alloc_new_message(TASK_ICM,message->ittiMsgHeader.messageId);

    icm_message->ittiMsgHeader=message->ittiMsgHeader;
    switch (message->ittiMsgHeader.messageId){
#define MESSAGE_DEF(iD, pRIO, sTRUCT, fIELDnAME) case iD: icm_message->ittiMsg.fIELDnAME = message->ittiMsg.fIELDnAME; break;
...
#undef MESSAGE_DEF
    ...
    // send copied message to ICM, if it is not RRC_SUBFRAME_PROCESS
    // ======================================================================
    // ======================================================================
    if ((icm_message->ittiMsgHeader.messageId) != (RRC_SUBFRAME_PROCESS)){
    t=&tasks[TASK_ICM];
        pthread_mutex_lock (&t->queue_cond_lock);
        int ret_icm=itti_send_msg_to_task_locked(TASK_ICM, instance, icm_message);
