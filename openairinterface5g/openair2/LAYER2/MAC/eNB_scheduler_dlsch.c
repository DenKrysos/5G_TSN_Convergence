    ...
    for (UE_id = UE_list->head; UE_id >= 0; UE_id = UE_list->next[UE_id]) {
      ...
      */
      ...
      if (round_DL != 8) {
        ...
        int sdu_length_total_cpy = sdu_length_total;
        int index_prio;
        int t = frameP*10+subframeP;
        int prio2lcid_array[] =  {13,12,11,10,9,8,7, 6,5,4, 3};

/*
        int prio2lcid_array1[] = {13,12,11,10,9,8,7, 4,5,6, 3};
        int prio2lcid_array2[] = {13,12,11,10,9,8,7, 5,6,4, 3};
        int prio2lcid_array3[] = {13,12,11,10,9,8,7, 6,4,5, 3};
        int prio2lcid_array4[] = {13,12,11,10,9,8,7, 3,3,3, 3};
*/


        int prio2lcid_array1[] = {13,12,11,10,9,8,7, 4,4,4, 3};
        int prio2lcid_array2[] = {13,12,11,10,9,8,7, 5,5,5, 3};
        int prio2lcid_array3[] = {13,12,11,10,9,8,7, 6,6,6, 3};
        int prio2lcid_array4[] = {13,12,11,10,9,8,7, 3,3,3, 3};
...
/*
        	case 0:{
        		memcpy(prio2lcid_array,prio2lcid_array1,sizeof(prio2lcid_array));
        	} break;
        	case 1:{
        		memcpy(prio2lcid_array,prio2lcid_array2,sizeof(prio2lcid_array));
        	} break;
        	case 2:{
        		memcpy(prio2lcid_array,prio2lcid_array3,sizeof(prio2lcid_array));
        	} break;
*/
...
*/
			...
			default:{
				memcpy(prio2lcid_array,prio2lcid_array4,sizeof(prio2lcid_array));
			} break;
...
//       } // end if
			...
			if (eNB_UE_stats->fi_ind){
				if ((eNB_UE_stats->fi_ind != 3)){	// condition: default bearer cannot overwrite, exp 2
					prio2lcid_array[6] = (int)eNB_UE_stats->fi_ind;
				}
			}

        							// (NB_RB_MAX - DTCH)
        for (index_prio = 0;index_prio < (NB_RB_MAX - DTCH);index_prio++){
        // for (lcid = NB_RB_MAX - 1; lcid >= DTCH; lcid--) {

          lcid = prio2lcid_array[index_prio];
          ...
          if (TBS - ta_len - header_length_total - sdu_length_total - 3 > 0) {
            ...
            if (rlc_status.bytes_in_buffer > 0) {
              ...
              printf("[eNB %d][USER-PLANE DRB] Frame %d , Subframe %d: DTCH->DLSCH, Requesting %d bytes from RLC (lcid %d, priority %d, total hdr len %d), %d bytes in RLC buffer, got %d bytes.\n",
                      module_idP,
                      frameP,
                      subframeP,
                      TBS - ta_len - header_length_total - sdu_length_total - 3,
                      lcid,
                      ue_sched_ctrl->priority[lcid],
                      header_length_total,
        ...
        } // end for (lcid = NB_RB_MAX - 1; lcid >= DTCH; lcid--)
        ...
        if (ta_len + sdu_length_total + header_length_total > 0) {
          ...
          // analyze SDUs
          printf("[DEBUG-MAC]\n\tnum_sdus: %d\n\tpadding: %d\n\tpost_padding:%d\n\tsdu_lengths: ",
        		  num_sdus,padding,post_padding);
          int il;
          for(il=0;il<NB_RB_MAX;il++){
        	  printf("%d ",sdu_lengths[il]);
          }
          printf("\n\tsdu_lcids: ");
          for(il=0;il<NB_RB_MAX;il++){
              printf("%d ",sdu_lcids[il]);
          }
          printf("\n");
          ...
          // ================================================================================
          // we concatenate all DTCH-RLC-PDUs to one coming from lcid "3"


          // iterate over all MAC-SDUs
          int index_mac;
          int position=0,header_pos=2,pdu_pos,num_rlc_sdus[NB_RB_MAX],rlc_sdus_total=0,buffer_pos=0;
          unsigned char rlc_header[100];
          unsigned char dlsch_buffer_new[MAX_DLSCH_PAYLOAD_BYTES];
          memset(dlsch_buffer_new,0,MAX_DLSCH_PAYLOAD_BYTES);
          memset(rlc_header,0,sizeof(rlc_header));
          memset(num_rlc_sdus,0,sizeof(num_rlc_sdus));
          int s_len;
          int e=0;
          int data_field_size;
          int fi,sn;
          int srb_len,num_srb=0;

          // write SDUs from lcids 0-2 into new buffer first
          for (index_mac=0;index_mac<num_sdus;index_mac++){
        	  if (sdu_lcids[index_mac]<DTCH){
        		  memcpy(&dlsch_buffer_new[buffer_pos],&dlsch_buffer[buffer_pos],sdu_lengths[index_mac]);
        		  buffer_pos += sdu_lengths[index_mac];
        		  num_srb++;
        	  }
          }
          position = buffer_pos;
          ...
          printf("[DEBUG-MAC] num_sdus: %d, num_srb: %d\n",num_sdus,num_srb);

          if (num_sdus){
          // write RLC header while iterating over all SDUs
          for (index_mac=num_srb;index_mac<(num_sdus+num_srb);index_mac++){
        	  // an RLC pdu
        	  e = (dlsch_buffer[position]&0x04)>>2;
        	  fi = (dlsch_buffer[position]&0x18)>>3;
        	  sn = ((dlsch_buffer[position]&0x03)<<8)+(dlsch_buffer[position+1]);

        	  printf("\t\tindex_mac: %d,    e = %d, fi = %d, sn = %d\n",index_mac,e,fi,sn);

        	  if (sdu_lengths[index_mac]==0){
        		  continue;
        	  }
        	  ...
        	  data_field_size = 0;
        	  if (e){
        		  // E bit is set, there are more than one data fields -> iterate over E/LI-fields
        		  num_rlc_sdus[index_mac] = 1;
        		  while (e){
        			  num_rlc_sdus[index_mac]++;
        			  if (!(num_rlc_sdus[index_mac]%2)){
        				  // even in this header
        				  pdu_pos+=2;
        				  s_len = ((dlsch_buffer[pdu_pos]&0x7f)<<4)+((dlsch_buffer[pdu_pos+1]&0xf0)>>4);
        				  e = (dlsch_buffer[pdu_pos]&0x80)>>7;
        			  } else {
        				  // odd in this header
        				  pdu_pos+=1;
        				  s_len = ((dlsch_buffer[pdu_pos]&0x07)<<8)+(dlsch_buffer[pdu_pos+1]);
        				  e = (dlsch_buffer[pdu_pos]&0x08)>>3;
        			  }
        			  data_field_size+=s_len;

        			  // write into new header
            		  if (!(rlc_sdus_total%2)){
            			  // even, start at first bit
            			  rlc_header[header_pos] = 128 + ((s_len)>>4);			// E + LI
            			  rlc_header[header_pos+1] = ((s_len&0x0000000f)<<4);	// LI
            			  header_pos++;
            		  } else {
            			  // odd, shift 4 bits
            			  rlc_header[header_pos] += (8+(s_len>>8));				// E + LI
            			  rlc_header[header_pos+1] = s_len & 0x000000ff;		// LI
            			  header_pos+=2;
            		  }
            		  rlc_sdus_total++;
        		  }
        		  // add length of last data field (rest of PDU)
        		  s_len = sdu_lengths[index_mac] - (pdu_pos+2-position) - data_field_size;
    			  // write into new header
        		  if (!(rlc_sdus_total%2)){
        			  // even, start at first bit
        			  rlc_header[header_pos] = 128 + ((s_len)>>4);			// E + LI
        		  ...
        		  rlc_sdus_total++;
        		  // write data fields into new dlsch buffer (without header)
        		  memcpy(&dlsch_buffer_new[buffer_pos],&dlsch_buffer[pdu_pos+2],sdu_lengths[index_mac]-(pdu_pos+2-position));
        		  buffer_pos+=sdu_lengths[index_mac]-(pdu_pos+2-position);

        	  } else {
        		  // just one data field in this RLC PDU
        		  num_rlc_sdus[index_mac] = 1;
        		  s_len = sdu_lengths[index_mac]-2;
        		  // write into new header
        		  if (!(rlc_sdus_total%2)){
        			  // even, start at first bit
        			  rlc_header[header_pos] = 128 + ((s_len)>>4);			// E + LI
        		  ...
        		  // write data field into new dlsch buffer (without header)
        		  memcpy(&dlsch_buffer_new[buffer_pos],&dlsch_buffer[pdu_pos+2],s_len);
        		  buffer_pos+=s_len;
        	  }


        	  position+=sdu_lengths[index_mac];
          } // end for

          // remove last E/LI-field and set second-last E-bit to 0
          //printf("[DEBUG-MAC] rlc_sdus_total: %d, now removing the last LI field\n",rlc_sdus_total);
          if (!(rlc_sdus_total%2)){
        	  // even
        	  rlc_header[header_pos-3] = rlc_header[header_pos-3]&0x7f; // set E to 0
        	  rlc_header[header_pos-2] = rlc_header[header_pos-2]&0xf0;	 // remove last E/LI-field
        	  rlc_header[header_pos-1] = 0;								 // remove last E/LI-field
        	  header_pos-=1;
          } else {
        	  // odd
        	  if (rlc_sdus_total>1){
        	  	  rlc_header[header_pos-3] = rlc_header[header_pos-3]&0xf7; 	// set E to 0
        	  }
        	  rlc_header[header_pos-1] = 0;									// remove last E/LI-field
        	  rlc_header[header_pos] = 0;									// remove last E/LI-field
        	  header_pos-=1;
          }
          ...
          sn = eNB_UE_stats->sn;
          if (eNB_UE_stats->fi_ind){
        	  // the first pdu is part of a segment, so set the high bit of FI
        	  fi = fi|0x00000002;
          }
          if ((fi==1)||(fi==3)){
        	  // set indicator to lcid of last pdu
        	  eNB_UE_stats->fi_ind = sdu_lcids[num_sdus+num_srb-1];
          } else {
        	  eNB_UE_stats->fi_ind = 0;
          }

          rlc_header[0] = rlc_header[0]+(fi<<3);	// same FI as last pdu

          rlc_header[0] = rlc_header[0]|(sn>>8);
          rlc_header[1] = sn&0x000000ff;
          if (rlc_sdus_total>1){					// set E
        	  rlc_header[0] = rlc_header[0]|0x04;
          }

          // concatenate new RLC header and new dlsch buffer to one MAC SDU
          memcpy(&dlsch_buffer_new[srb_len+header_pos],&dlsch_buffer_new[srb_len],buffer_pos-srb_len);
          memcpy(&dlsch_buffer_new[srb_len],rlc_header,header_pos);
          memset(dlsch_buffer,0,MAX_DLSCH_PAYLOAD_BYTES);
          memcpy(dlsch_buffer,dlsch_buffer_new,buffer_pos+header_pos);

          // change MAC header information
          num_sdus = num_srb + 1;
          memset(&sdu_lengths[num_srb],0,sizeof(uint16_t)*(NB_RB_MAX-num_srb));
          sdu_lengths[num_srb] = buffer_pos + header_pos;
          memset(&sdu_lcids[num_srb],0,sizeof(uint16_t)*(NB_RB_MAX-num_srb));
          sdu_lcids[num_srb] = DTCH;
          sdu_length_total = sdu_length_total_cpy+buffer_pos+header_pos;
          ...
          printf("[DEBUG-MAC] num_rlc_sdus found by algorithm: ");
          for(il=0;il<NB_RB_MAX;il++){
              printf("%d ",num_rlc_sdus[il]);
          }
          printf("\n");
