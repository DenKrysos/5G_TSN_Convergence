...
/*
      uint8_t teid[4];
      memcpy(teid,udp_data_ind_p->buffer+4,4);
      printf("[DEBUG-GTP] received downstream with TEID %x %x %x %x for Task ID %d, %d bytes\n",
...
/*
      // hardcode change of teid to lead downlink traffic to dedicated bearers
      uint8_t src_addr_a[4];
      memcpy(src_addr_a,udp_data_ind_p->buffer+20,4);
      int src_addr = (src_addr_a[0] << 24) + (src_addr_a[1] << 16) + (src_addr_a[2] << 8) + (src_addr_a[3]);
      printf("[DEBUG-UDP] src_addr int: %d\n",src_addr);
      if (src_addr == 312374690 || src_addr == 314154196 || src_addr == 597518210){  // ecosia.org
    	  printf("[DRB] changing DL traffic to enter dedicated bearer\n");
      	  teid[0] = 0x8d;
      	  teid[1] = 0x9a;
      	  teid[2] = 0x30;
      	  teid[3] = 0x66;
      	  memcpy(udp_data_ind_p->buffer+4,teid,4);
      }
*/
...
/*
        uint8_t teid[4];
        memcpy(teid,&udp_data_req_p->buffer[udp_data_req_p->buffer_offset]+4,4);
        printf("[DEBUG-GTP] sending upstream to TEID %x %x %x %x\n",
        		teid[0],teid[1],teid[2],teid[3]);
*/


        //==========================================================
        // hardcoded traffic change for Upstream UDP (packet_terminal)
        char *address_printable="192.168.178.41";	// eNB IPv4 address
        in_addr_t to_addr;
        inet_pton(AF_INET,address_printable,&to_addr);
        peer_addr.sin_port=htons(2150);
        peer_addr.sin_addr.s_addr=to_addr;
        //==========================================================
