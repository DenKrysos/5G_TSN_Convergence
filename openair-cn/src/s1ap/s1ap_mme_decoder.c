...
s1ap_mme_decode_successfull_outcome (
  ...
  switch (successfullOutcome_p->procedureCode) {
    ...
    case S1ap_ProcedureCode_id_E_RABSetup:{
    	free_wrapper ((void**) &message_string);
    	// send itti message to TASK_DRB to set the pending bearer to active state
    	message_p = itti_alloc_new_message(TASK_S1AP,S1AP_NAS_DL_DATA_REQ);
    ...
    default: {
    	printf("[DEBUG] S1AP Unknown procedure ID (%ld) for successfull outcome message\n",successfullOutcome_p->procedureCode);
