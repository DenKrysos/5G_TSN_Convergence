  ...
  const void       const* opt_relay_node_indicator)  // unused
{
  ...
  // ==============================================
  // send initial UE msg (copy) to TASK_DRB

  MessageDef * message_cpy = NULL;
  message_cpy = itti_alloc_new_message(TASK_S1AP, MME_APP_INITIAL_UE_MESSAGE);
  itti_mme_app_initial_ue_message_t * msg = &message_cpy->ittiMsg.mme_app_initial_ue_message;
  memcpy(msg,&message_p->ittiMsg.mme_app_initial_ue_message,sizeof(itti_mme_app_initial_ue_message_t));

  itti_send_msg_to_task(TASK_DRB, INSTANCE_DEFAULT, message_cpy);
  // ==============================================
