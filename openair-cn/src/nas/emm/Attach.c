  ...
  const_bstring esm_msg_pP)
{
  ...
  if ((rc != RETURNerror) && (esm_sap.err == ESM_SAP_SUCCESS)) {
    ...
    // ===========================================================================================
    // send message with ue_id to TASK_DRB
    MessageDef *message_p=NULL;
    itti_attach_forward_drb_t *itti_attach_forward_drb_p;
    message_p = itti_alloc_new_message(TASK_NAS_MME, ATTACH_FORWARD_DRB);

    itti_attach_forward_drb_p = &message_p->ittiMsg.attach_forward_drb;
    itti_attach_forward_drb_p->emm_ctx = emm_ctx;

    itti_send_msg_to_task(TASK_DRB, INSTANCE_DEFAULT, message_p);
    // ===========================================================================================
